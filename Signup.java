...
public class Signup extends JFrame implements ActionListener{
	...
	comboBox.setModel(new DefaultComboBoxModel(new String[] { "Your childhood school?", "Your Lucky Number?",
			"Your pet name?", "Your childhood Name ?" }));
    ...
    public void actionPerformed(ActionEvent ae){
        ...
        if (ae.getSource() == b1) {
            // Validate the name format (first name and last name)
            String name = textField_1.getText().trim();
            if (!name.matches("^[a-zA-Z]+\\s[a-zA-Z]+$")) {
                JOptionPane.showMessageDialog(null, "Please enter your name in the form of First Name and then Last Name");
                return;
            }
